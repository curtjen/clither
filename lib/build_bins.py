...
def build_bins():
  ...
  for dir in addon_dirs:
    #TODO(xnz): We will want to abstract this part out, as we use it a lot.
    print('Run ders on: ' + dir)
    ...
    if os.path.exists(config_path):
      ...
      bins = config.setdefault('bins')

      if not bins:
        print('File does not have rcs section: ' + config_path)
        return

      pathing = ['$PATH+=:{0}/{1}'.format(addon_path, bin) for bin in bins]
      
      pathing = '\n'.join(pathing)

      print(pathing)
      # _build_rc_files(bins, addon_path)
    else:
